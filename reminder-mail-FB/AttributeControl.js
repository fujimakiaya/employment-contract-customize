...
class AttributeControl {
  ...
  set(fieldcode, attribute_type, attribute) {
    try {
      ...
      if (!targetElement) {
        console.error(
          `set_attribute error\n  fieldcode=${fieldcode}\n  detail=target element not found`
        );
        return;
      }
  ...
  update_checkbox(fieldcode, dataObject) {
    // dataObject = [{value:???, code:???},]
    const targetElement = this.#get_element_byfieldcode(fieldcode);
    if (document.getElementById(fieldcode))
      document.getElementById(fieldcode).remove();
    ...
    function updateCheckedValues() {
      ...
      for (let i = 0; i < checkboxes.length; i++) {
        if (checkboxes[i].getElementsByTagName("input")[0].checked) {
          checkedValues.push(
            checkboxes[i].getElementsByTagName("input")[0].value
          );
        }
      }
