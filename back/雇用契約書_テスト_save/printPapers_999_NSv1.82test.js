(function () {
    ...
    if (title == "労働者名簿") {
      ...
      if (batch_params) {
        try {
          ...
          // window.close();
          // 非同期処理が完了した後に遷移
          window.location.href = `https://5ea2a167.viewer.kintoneapp.com/public/nkrfsv2-8-newkv-for-workerlist-3202-test?companyId=${companyId}`;
        } catch (error) {
  ...
  kviewer.events.on("record.show", function (state) {
    ...
    if (ledgerNames[0] == "退職証明書" && DoubleLineflag == true) {
      ...
      aTag.href = `https://5ea2a167.viewer.kintoneapp.com/public/nkrfsv2-8-newkv-for-workerlist-3202-test?companyId=${companyId}&name=${userName}`;
