...
(function () {
  ...
  kviewer.events.on("record.show", async function (state) {
    ...
    const param = `?additionalFilters=[{"field":"社員No","sign":"=","value":"${empId}","with":"and"}]&companyId=${companyId}&name=${userName}`;
    ...
    pButton1.addEventListener("click", async function () {
      try {
        ...
        const foundIdSet = new Set(
          existData.records.map((r) => r["社員No"]?.value).filter(Boolean),
        );
        // empIds の順番を基準に、存在するものだけ残す
        const foundIds = empIds.filter((id) => foundIdSet.has(id));
        ...
        if (isConfirmed) {
          ...
          window.location.href = `https://5ea2a167.viewer.kintoneapp.com/public/nkrfsv2-8-newkv-print-workerinfo-3218-999?companyId=${companyId}&batch=${batch_params}&name=${userName}`;
        } else {
          hideSpinner();
        }
      } catch (e) {
